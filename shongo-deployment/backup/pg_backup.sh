...
if [ ! $DB_NAME_LIST ]; then
    ...
    DB_NAME_CLAUSE="false"
	for DB_NAME in ${DB_NAME_LIST//,/ }
    do
    	DB_NAME_CLAUSE="$DB_NAME_CLAUSE or datname = '$DB_NAME'"
    done
fi;
...
SCHEMA_ONLY_QUERY="select datname from pg_database where ($DB_NAME_CLAUSE) and (false $SCHEMA_ONLY_CLAUSE) order by datname;"
...
FULL_BACKUP_QUERY="select datname from pg_database where not datistemplate and datallowconn and ($DB_NAME_CLAUSE) $EXCLUDE_SCHEMA_ONLY_CLAUSE order by datname;"
