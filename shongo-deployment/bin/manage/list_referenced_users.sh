...
# list users
$BIN/../shongo-client-cli.sh src --connect $HOST --scripting --cmd "list-referenced-users" \
    | sed '/userId/{N;s/\n//;}' \
    | grep userId \
...
do
    if [[ $user_id -ne 0 ]] ; then
        result=$($BIN/../shongo-client-cli.sh src --connect $HOST --scripting --cmd "get-user $user_id" \
            | tr -d '\n' \
            | grep "\[ " \
