...
SELECT     
    ...
    'req:' || abstract_reservation_request.latest_id AS request_id,
    
    /* Real request version id (modified versions of the same request have different version_id) */
    'req:' || abstract_reservation_request.id AS version_id,
...
LEFT JOIN alias_specification AS room_name_specification ON room_name_specification.id IN (
    ...
    SELECT specification.id FROM (
        /* This request */
        SELECT abstract_reservation_request.specification_id AS id        
        UNION ALL
        /* This request in alias_set_specification */
        SELECT reused_reservation_request.specification_id
        ...
        UNION ALL
        /* Reused request */
        SELECT alias_set_specification_alias_specifications.alias_specification_id
        FROM alias_set_specification
        LEFT JOIN alias_set_specification_alias_specifications ON alias_set_specification_alias_specifications.alias_set_specification_id = alias_set_specification.id
        WHERE alias_set_specification.id = abstract_reservation_request.specification_id
        UNION ALL
        /* Reused request in alias_set_specification */
        SELECT alias_set_specification_alias_specifications.alias_specification_id
	FROM allocation AS reused_allocation
        LEFT JOIN abstract_reservation_request AS reused_reservation_request ON reused_reservation_request.id = reused_allocation.abstract_reservation_request_id   
        LEFT JOIN alias_set_specification_alias_specifications ON alias_set_specification_alias_specifications.alias_set_specification_id = reused_reservation_request.specification_id
        WHERE reused_allocation.id = abstract_reservation_request.reused_allocation_id
    ) AS specification    
