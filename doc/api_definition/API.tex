...
\begin{ApiClassAttributes}
\ApiClassAttribute{duration}{Duration}{\ApiOptionalDefault{P0}}
\end{ApiClassAttributes}
...
\ApiClassAttribute{period}{Duration}{\ApiOptional}
...
\ApiClassAttribute{technology}{Technology}{\ApiOptional} Technology type of the resource, see |Technology|. 
\ApiClassAttribute{translation}{Translation}{\ApiOptional} Translation type of the gateway resource, see |Translation|. 
...
\ApiClassAttribute{resources}{String[]}{\ApiOptional} List of child resources identifiers (e.g., the resource is physical room and |resources| contains all videoconferencing devices in the room). 
\end{ApiClassAttributes}
...
Represents a complete reservation class that is used for creating and modifying reservations and for retrieving detailed reservations information.
\begin{ApiClassAttributes}
...
\ApiClassAttribute{reservations}{String[]}{\ApiOptional} List of child reservations identifiers. All allocated resources from child reservations become part of the parent reservation.
...
Represents successfully allocated time slots with concrete allocated resources.
\begin{ApiClassAttributes}
\ApiClassAttribute{slotResources}{Map<TimeSlot, String[]>}{\ApiReadOnly} Map of allocated resources where key is allocated time slot and value is list of allocated resources for the time slot (for different time slots there can be different allocated resources).
\end{ApiClassAttributes}


\ApiClass{ReservationSummary}{}
Represents a summary of a reservation. This class is used when listing a lot of reservations information by one request.
\begin{ApiClassAttributes}
\ApiClassAttribute{id}{String}{}
\ApiClassAttribute{type}{ReservationType}{}
\ApiClassAttribute{description}{String}{}
\ApiClassAttribute{date}{AbsoluteDateTime}{} Specifies the first future date/time when the reservation takes place.
\end{ApiClassAttributes}
...
\ApiCmd{Resource[] listResources(SecurityToken token, Map filter)}
List of resources managed by Shongo, that a user with given |token| is entitled to see and that meet the resource |filter| which contains only attributes defined in |Resource| class.
...
\ApiCmd{ReservationSummary[] listReservationSummary(SecurityToken token, Map filter)}
