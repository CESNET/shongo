...
Represented as 64-bit signed integer, it has semantics of Unix time since epoch with resolution of a second.
...
\APIcmd{Reservation makeReservation(SecurityToken token, String description, Date date, Duration duration, Resources[] resources)}

\APIcmd{Reservation makePeriodicReservation(SecurityToken token, String description, PeriodicDate date, Duration duration, Resources[] resources)}

\APIcmd{Reservation makePermanentReservation(SecurityToken token, String description, PeriodicDate date, Duration duration, Resources[] resources)}

\APIcmd{releaseReservation(SecurityToken token, Reservation)}
