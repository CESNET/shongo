...
Represents a base class for all specific specifications.

\ApiClass{CompartmentSpecification}{Specification}
Represents a group of endpoints which should be interconnected by the Scheduler.
\begin{ApiClassAttributes}
\ApiClassAttribute{specifications}{List<Specification>}{\ApiRequired} List of specifications which should be interconnected in the compartment.
\ApiClassAttribute{callInitiation}{CallInitiation}{\ApiOptional} Default \ApiCode{CallInitiation} in the compartment.
\end{ApiClassAttributes}

\ApiClass{ExternalEndpointSpecification}{Specification}
Represents a specification for external endpoint resource(s) that is/are requested for a reservation.
\begin{ApiClassAttributes}
...
\ApiClass{ExistingEndpointSpecification}{Specification}
Represents a specification for an existing resource (known to controller) that is requested for a reservation.
\begin{ApiClassAttributes}
\ApiClassAttribute{resourceIdentifier}{String}{\ApiRequired} Identifier of the requested endpoint resource.
\end{ApiClassAttributes}

\ApiClass{LookupEndpointSpecification}{Specification}
Represents a specification for any existing endpoint resource with given |technology| in the controller resource database.
\begin{ApiClassAttributes}
\ApiClassAttribute{technology}{Technology}{\ApiRequired} Technology of the resource.
\end{ApiClassAttributes}

\ApiClass{AliasSpecification}{Specification}
Represents a specification that requests a reservation for an alias for a video conferencing device.
\begin{ApiClassAttributes}
