...
The ontology used by all agents consists of concepts, predicates, and agent actions. A special type is defined for each concept, predicate or agent action, implementing the |Concept|, |Predicate| or |AgentAction| interface, respectivelly, from the |jade.content| package. Each of the commands specified in this API document is defined by a class implementing |AgentAction|, declaring all the command arguments as attributes accessed by public getters and setters. For example, the |setMicrophoneLevel(int level)| command, defined in section \ref{sect:connector-endpoint-api}, might be specified by the following class:
\begin{verbatim}
...
\end{verbatim}
Note that all classes of object arguments of any command should be defined within the \\
|cz.cesnet.shongo.ontology| interface and tagged by the |Concept| interface for the |ShongoOntology| class to be able to find them and comprise them in the ontology.

The agent receiving a command should always send a reply as an |inform| \cite{FIPA-ComActSpec} message. In case of commands without any return value, a |Done| predicate from the package |jade.content.onto.basic| should be sent as a reply, denoting a successful command execution. When a return value is expected, a |Result| predicate, defined by \cite{FIPA-SL}, is sent, filled with the value to be returned. The same requirements apply to the class of the object to be returned as for command object arguments -- the class must reside within the |cz.cesnet.shongo.ontology| interface and be tagged by the |Concept| interface.
...
\ApiClass{RoomInfo}{}
A brief info about a room at a server.
\begin{ApiClassAttributes}
\ApiClassAttribute{name}{String}{\ApiRequired} Name of the room.
\TODO{some more attributes? whether it is currently in use, number of participants, the next usage...}
\end{ApiClassAttributes}
