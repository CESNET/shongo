...
\ApiFailure{17} The attribute cannot be read because it is write-only, the |faultString| specifies which attribute in which class.
...
\ApiFailure{99} A part of request is not implemented yet. The domain controller log must be checked for more details.
...
Relative date is calculated as referenced date/time increased by |Period|.
...
\ApiClass{Capability}{}
Base class for all specific capabilities which the device can support.
\begin{ApiClassAttributes}
...
\ApiClassAttribute{parentIdentifier}{String}{\ApiRequired} A parent resource identifier in which is the resource located (e.g., identifier of a physical room).
...
Capability for devices which allows for creating virtual rooms for connecting multiple endpoint.
\begin{ApiClassAttributes}
\ApiClassAttribute{portCount}{Integer}{\ApiRequired}
Maximum port count for all virtual rooms.
\end{ApiClassAttributes}
...
\begin{ApiClassAttributes}
\ApiClassAttribute{identifier}{String}{}
