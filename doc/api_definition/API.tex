...
\begin{ApiEnumValues}
\ApiEnumValue{SingleParticipant}{only a single, fixed participant is viewed}
\ApiEnumValue{VoiceSwitchedSingleParticipant}{only a single, currently speaking participant is viewed}
...
\TODO{How to upload/modify calendars for Endpoint/Multipoint device types? connectors not involved in calendars regarding reservations; this TODO item mentions rather a plan what to do to prevent the need for immediate commands just on time of an event; resolve handling of errors, e.g. when a recording upload fails, the following delete command should not be executed\ldots}
