...
\ApiClass{User}{}
Each user that accesses shongo or participates in shongo managed videoconference should be uniquely identified by |User| definition.
\begin{ApiClassAttributes}
\ApiClassAttribute{id}{String}{\ApiReadOnly} 
\TODO{Perhaps eduID identity?}
\end{ApiClassAttributes}
...
\ApiCmd{Reservation modifyReservation(SecurityToken token, String reservationId, AttributeMap<Reservation> attributes)}
Modify the reservation with specified |reservationId|.

\ApiCmd{deleteReservation(SecurityToken token, String reservationId)}
Release the reservation with specified |reservationId| and all children reservations.
...
\ApiClass{Event}{}
Represents abstract event that takes place on specified |date| and takes |duration| period of time.
\begin{ApiClassAttributes}
\ApiClassAttribute{date}{AbsoluteDate}{\ApiRequired}
