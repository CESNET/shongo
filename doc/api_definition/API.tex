...
Communication among controllers and connectors is implemented using JADE \cite{jade}. All messages are encoded using the FIPA SL content language \cite{FIPA-SL}. An ontology, called \texttt{ShongoOntology}, is used by communicating agents to give the same meaning to the symbols used in messages. This section describes the way commands defined by this API are composed to messages and interpreted by Shongo agents.

The ontology used by all agents consists of concepts, predicates, and agent actions. A special type is defined for each concept, predicate or agent action, implementing the \texttt{Concept}, \texttt{Predicate} or \texttt{AgentAction} interface, respectivelly, from the \texttt{jade.content} package. Each of the commands specified in this API document is defined by a class implementing \texttt{AgentAction}, declaring all the command arguments as attributes accessed by public getters and setters. For example, the \texttt{setMicrophoneLevel(int level)} command, defined in section \ref{sect:connector-endpoint-api}, might be specified by the following class:
\begin{verbatim}
...
\end{verbatim}
The \texttt{setMicrophoneLevel} call implementation instantiates a new \texttt{SetMicrophoneLevel} object, sets up the \texttt{level} attribute, and passes the object to a controller agent content manager to send it to an endpoint as a \texttt{request} communicative act \cite{FIPA-ComActSpec}. The corresponding endpoint agent creates the \texttt{SetMicrophoneLevel} object received from the controller agent and implements the requested functionality according to it. The message sent during such a call might be similar to the following:
\begin{verbatim}
(REQUEST
...
\end{verbatim}
Note that all classes of object arguments of any command should be defined within the \texttt{cz.cesnet.shongo.ontology} interface and tagged by the \texttt{Concept} interface for the \texttt{ShongoOntology} class to be able to find them and comprise them in the ontology.

The agent receiving a command should always send a reply as an \texttt{inform} \cite{FIPA-ComActSpec} message. In case of commands without any return value, a \texttt{Done} predicate from the package \texttt{jade.content.onto.basic} should be sent as a reply, denoting a successful command execution. When a return value is expected, a \texttt{Result} predicate, defined by \cite{FIPA-SL}, is sent, filled with the value to be returned. The same requirements apply to the class of the object to be returned as for command object arguments -- the class must reside within the \texttt{cz.cesnet.shongo.ontology} interface and be tagged by the \texttt{Concept} interface.
...
\section{Common API}
\TODO{read through the use cases and complete the API}
