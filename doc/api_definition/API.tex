...
\item |faultCode = 1| \\ The value of \ApiValue{class} member in a |struct| is wrong, the |faultString| specifies which \ApiValue{class} value.
\item |faultCode = 2| \\ The value of an enum member attribute is wrong., the |faultString| specifies which value for which enum type.
\end{compactitem}
...
If time slot contains |PeriodicDate|, all periodic events can be listed by evaluating time slot to |TimeSlot[]|, where only |AbsoluteDate| are used.
...
\ApiClassAttribute{technology}{Technology}{} Technology type of the resource, see |TechnologyType|. 
\ApiClassAttribute{translation}{Translation}{} Translation type of the gateway resource, see |TranslationType|. 
...
\begin{ApiClassAttributes}
\ApiClassAttribute{id}{String}{\ApiRequired} Reservation unique identifier as defined in \UCref{rsv:identification}.
...
\ApiClassAttribute{users}{User[]}{} List of users that will participate in reserved videoconference.
\end{ApiClassAttributes}
