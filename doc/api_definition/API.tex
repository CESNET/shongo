...
\begin{ApiClassAttributes}
\ApiClassAttribute{duration}{Period}{\ApiOptionalDefault{PT0S}}
\end{ApiClassAttributes}
...
\ApiClass{Period}{}
Represented as ISO8601 duration (e.g., \ApiValue{P3Y6M4DT12H30M5S} which is \textit{3 years, 6 months, 4 days, 12 hours, 30 minutes, and 5 seconds} or \ApiValue{P4W} which is \textit{4 weeks}). The zero duration is represented by \ApiValue{PT0S} value.


\ApiClass{TimeSlot = Pair<DateTime, Period>}{}
...
\ApiClassAttribute{technology}{Technology}{}
...
\begin{ApiClassAttributes}
\ApiClassAttribute{slotResources}{Map<TimeSlot, String[]>}{\ApiReadOnly} Map of allocated resources where key is allocated time slot and value is list of allocated resources identifiers for the time slot (for different time slots there can be different allocated resources).
\end{ApiClassAttributes}
...
\ApiCmd{TimeSlot[] findReservationAvailableTime(SecurityToken token, Period duration, Resource[] resources, boolean interDomain)}
