...
\ApiClassAttribute{rules}{Rule[]}{\ApiRequired} List of rules which defines what channels is capability able to process. Rule can be one of the following types:
\begin{compactitem}
\item |Enable| means that the device is able to process specified channel.
\item |Disable| means that the device is not able to process specified channel.
\end{compactitem}
For |Input|, |Output|, |Recording| and |Streaming| capability type one |channel| must be specified. For |Translation| capability type channel |from| and |to| must be specified.
\end{ApiClassAttributes}
...
capability.rules = {
    { type = Enable, from = H323|SIP:*:H261:*x*, to = H323|SIP:*:H261:*x* },
    { type = Disable, from = *:*:*:1080p, to = *:*:*:1080p },
}
\end{verbatim}
