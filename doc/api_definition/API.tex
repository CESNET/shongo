...
Represents map of attributes for an object of type |T|. Attribute is pair of |name| and |value|, where name is always |String| and value can be of any allowed type. The map for a type |T| can contain only the attributes that are defined in type |T|.
...
\ApiClass{Date}{}
It serves only as base class for |AbsoluteDate|, |RelativeDate| and |PeriodicDate|.


\ApiClass{AbsoluteDate}{Date}
...
\ApiClassAttribute{date}{String}{\ApiRequired}
\end{ApiClassAttributes}

\ApiExample We want to define a maximum future time that can be specified as part of some request and we know the precise date (e.g., the date since a resource will not be available for any request). We can specify it by |AbsoluteDate|:
\begin{verbatim}
absoluteDate.date = 20121231
\end{verbatim}

\ApiClass{RelativeDate}{Date}
...
\ApiExample We want to define the maximum amount of time since the request moment (e.g., the user can specify dates which are at most 4 months ahead). We can specify it by |RelativeDate| as follows:
\begin{verbatim}
relativeDate.duration = P4M
\end{verbatim}
...
\begin{ApiClassAttributes}
\ApiClassAttribute{start}{AbsoluteDate}{\ApiRequired}
...
\ApiClassAttribute{end}{AbsoluteDate}{}
...
\ApiClass{TimeSlot = Pair<Date, Duration>}{}
Time slot is pair of |Date| and |Duration|. For reservation purposes, the array |TimeSlot[]| should be used to provide the ability to reserve multiple date and times with different durations (e.g., on every Monday from 14:00 to 15:00 and every Thursday from 16:00 to 18:00).

If time slot contains |PeriodicDate|, all periodic events can be listed by evaluating time slot to |TimeSlot[]|, where only |AbsoluteDate|s are used.
...
\ApiClassAttribute{resources}{Resource[]}{} List of child resources. If the resource is physical room, |resources| contains all videoconferencing devices in the room. 
\end{ApiClassAttributes}
...
\ApiClassAttribute{timeSlots}{TimeSlot[]}{\ApiRequired} Requested time slots for specified resources.
\ApiClassAttribute{description}{String}{} Long reservation description.
\ApiClassAttribute{reservations}{Reservation[]}{} List of child reservations. All allocated resources from child reservations become part of the parent reservation.
\ApiClassAttribute{requestedResources}{Resource[]}{} List of requested resources by this reservation.
\ApiClassAttribute{allocatedResources}{Resource[]}{\ApiReadOnly} List of allocated resources by scheduler for this reservation. Does not include the child reservations resources, they must be looked up from |reservations|.
\ApiClassAttribute{users}{User[]}{} List of users that will participate in the videoconference.
\end{ApiClassAttributes}
...
\ApiCmd{Resource modifyResource(SecurityToken token, String id, AttributeMap<Resource> attributes)}
Modify the resource with specified |id|. That operation is permited only when the user with given |token| is the resource owner.

\ApiCmd{deleteResource(SecurityToken token, String id)}
Delete the resource with specified |id| from Shongo management. That operation is permited only when the user with given |token| is the resource owner and only when the resource is not used in any future reservation.
...
\ApiCmd{Reservation createReservation(SecurityToken token, AttributeMap<Reservation> attributes)}
Create a new reservation. The new reservation identifier is included in returned object as |id| attribute.

\ApiCmd{Reservation modifyReservation(SecurityToken token, String id, AttributeMap<Reservation> attributes)}
Modify the reservation with specified |id|.

\ApiCmd{deleteReservation(SecurityToken token, String id)}
Release the reservation with specified |id| and all children reservations.
