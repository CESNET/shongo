...
% Next empty linea are intended to be blank (wraps the text 
% to the next line even if the \APIxxx is not followed by empty line)
\newcommand{\APIcmd}[1]{%
\item \APIcode{#1}%
...
\begin{APIattrs}
\APIattr{duration}{Duration} Default value is zero duration \verb|P0|.
\end{APIattrs}
...
\begin{APIattrs}
\APIattr{id}{String} Resource unique identifier as defined in \UCref{res:identification}.
\APIattr{name}{String} Short name which desribes the resource.
\APIattr{managed}{Boolean} Specify if resource is managed and scheluded by Shongo.
\APIattr{type}{ResourceType} Type or capabilities of the resource, see |ResourceType|. 
\APIattr{description}{String} Long description depicting the resource.
\APIattr{maxFuture}{AutoDate} The maximum future time for reservations as defined in \UCref{rsv:max-future}.
\end{APIattrs}
