...
\newcommand{\ApiClassAttribute}[3]{\ApiItem{\ApiCode{#2} \ApiCode{#1}
  \ifx\ApiRequired#3\relax \ApiRequired \else \ApiOptional \fi}%
}
...
\ApiClass{Date}{}
It serves only as base class for |AbsoluteDate|, |RelativeDate| and |PeriodicalDate|.
...
Time slot is pair of |Date| and |Duration|. |PeriodicDate| can be evaluated to |TimeSlot[]| that contains only |AbsoluteDate| instances of |Date|.
...
\begin{ApiClassAttributes}
\ApiClassAttribute{from}{TechnologyType[]}{\ApiRequired}
Input technologies that server is able to transform.
\ApiClassAttribute{to}{TechnologyType[]}{\ApiRequired}
...
\ApiClassAttribute{name}{String}{\ApiRequired} Short name which desribes the resource.
...
\ApiClassAttribute{resources}{Resource[]}{} List of requested/reserved resources.
\end{ApiClassAttributes}
