...
\ApiItem{\ApiCode{RoomSummary getRoomSummary(SecurityToken token, String roomId)}}
...
\ApiItem{\ApiCode{Collection<RoomUser> listParticipants(SecurityToken token, String roomId)}}

\ApiItem{\ApiCode{RoomUser getParticipant(SecurityToken token, String roomId, String roomUserId)}}
Gets user information and settings in a room.

\ApiItem{\ApiCode{dialParticipant(SecurityToken token, String roomId, String deviceAddress)}}
\ApiItem{\ApiCode{dialParticipant(SecurityToken token, String roomId, Alias alias)}}
Dials a device, multipoint or endpoint. Dialing a client is available only on \textbf{H.323} and \textbf{SIP}.

\ApiItem{\ApiCode{modifyParticipant(SecurityToken token, String roomId, String roomUserId, Map attributes)}}
Modifies user settings in the room (suitable for setting
microphone/playback level, muting/unmuting, user layout\ldots).

\ApiItem{\ApiCode{disconnectParticipant(SecurityToken token, String roomId, String roomUserId)}}
...
\ApiItem{\ApiCode{muteParticipant(SecurityToken token, String roomUserId)}}
Mutes a user in a room.

\ApiItem{\ApiCode{unmuteParticipant(SecurityToken token, String roomUserId)}}
Unmutes a user in a room.

\ApiItem{\ApiCode{setParticipantMicrophoneLevel(SecurityToken token, String roomUserId, int level)}}
...
\ApiItem{\ApiCode{setParticipantPlaybackLevel(SecurityToken token, String roomUserId, int level)}}
Sets playback audio level of a user in a room to a given value. Note that the implementation differs between multipoint and endpoint types of devices. On an endpoint, the playback level is set using the device amplifier, while calling this on a multipoint device results in software adaptation of the output sound data (which may result in a distorted sound). The range for \ApiCode{level} is 0 to 100. The implementing connector adapts this value to the range for its managed device.

\ApiItem{\ApiCode{enableParticipantVideo(SecurityToken token, String roomUserId)}}
Enables video from a user in a room.

\ApiItem{\ApiCode{disableParticipantVideo(SecurityToken token, String roomUserId)}}
...
\ApiItem{\ApiCode{String dial(SecurityToken token, String address)}}
\ApiItem{\ApiCode{String dial(SecurityToken token, Alias alias)}}
Dials a server. Returns the device's identification of the call so that the call may be referred to by other methods.

\ApiItem{\ApiCode{hangUp(SecurityToken token, String callId)}}
Hangs up a call. The \ApiCode{callId} argument is that previously returned by the \ApiCode{dial} method.

\ApiItem{\ApiCode{hangUpAll(SecurityToken token)}}
Hangs up all calls.

\ApiItem{\ApiCode{standBy(SecurityToken token)}}
Sets the device to standby mode.

\ApiItem{\ApiCode{resetDevice(SecurityToken token)}}
Resets the device.

\ApiItem{\ApiCode{mute(SecurityToken token)}}
Mutes the endpoint.

\ApiItem{\ApiCode{unmute(SecurityToken token)}}
Unmutes the endpoint.

\ApiItem{\ApiCode{setMicrophoneLevel(SecurityToken token, int level)}}
...
\ApiItem{\ApiCode{setPlaybackLevel(SecurityToken token, int level)}}
...
\ApiItem{\ApiCode{enableVideo(SecurityToken token)}}
Enables video from the endpoint.

\ApiItem{\ApiCode{disableVideo(SecurityToken token)}}
Disables video from the endpoint.

\ApiItem{\ApiCode{startPresentation(SecurityToken token)}}
Starts the presentation mode (turns on the media stream).

\ApiItem{\ApiCode{stopPresentation(SecurityToken token)}}
