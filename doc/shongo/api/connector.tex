...
Modifies a room identified by \ApiCode{roomId}. The \ApiCode{attributes} and \ApiCode{options} maps specify \ApiCode{Room} attribute names and options, respectivelly, mapped to new values. For available \ApiCode{options}, see the \ApiCode{Room.Option} enumeration (options are used as keys, values should be of the type specified for the option). In the \ApiCode{attributes} map, any \ApiCode{Room} attribute name (\ApiCode{name}, \ApiCode{portCount}, \ldots) may be used as the key mapped to a value of the corresponding type.
