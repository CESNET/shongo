...
\item \textbf{Person} \\
  ...
  \end{compactenum}
\end{compactenum}
A reservation request is made by an user in a domain controller. The domain controller then
...
\end{compactenum}
Every domain controller is running a scheduler which process complete meeting requests and tries to schedule them. If the scheduling was successful, the result is an allocated meeting.
...
\CodeStyle{
  ...
  ReservationRequest, PersonRequest, GroupRequest
}
