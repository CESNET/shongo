...
{
    RESULT=$($BIN/client_cli.sh --connect $CONTROLLER --root --scripting --cmd "status" 2> /dev/null)
...
function check_connector
{
    ...
    RESULT=$($BIN/client_cli.sh --connect $CONTROLLER --root --scripting --cmd "list-connectors" 2> /dev/null)
    if [ -z "$RESULT" ]; then
        echo "FAILED (controller not available at '$CONTROLLER')"
        exit 2
    fi
