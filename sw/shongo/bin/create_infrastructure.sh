...
RUN_CLIENT_CLI="cat"
RUN_CLIENT_CLI="./client_cli.sh --connect $CONTROLLER --scripting"
...
<<COMMENT
bin/client_cli.sh --connect meetings.cesnet.cz --cmd "control-resource 4 list-rooms" \
    | grep "^|.\+|.\+|" \
...
<<COMMENT
bin/client_cli.sh --connect meetings.cesnet.cz --cmd "control-resource 24 list-rooms" \
    | grep "^|.\+|.\+|" \
    | grep -v "exe:\|-+-\|| Description" \
    | sed -r -e "s/^\|[^\|]+\|[^\|]+\|[^\|]+\| *\/([^\|]+)\/ *\|.*/\1/g" \
    | grep -v "^|" \
