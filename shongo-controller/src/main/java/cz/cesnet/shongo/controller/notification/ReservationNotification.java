...
public abstract class ReservationNotification extends AbstractReservationRequestNotification
{
    ...
    protected NotificationMessage renderMessage(Configuration configuration, NotificationManager manager)
    {
        ...
        addEvent(calendar, context, method);
        // TODO: ok to create new?
        NotificationState notificationState = new NotificationState();
        message.addAttachment(new iCalendarNotificationAttachment("name.ics", calendar, notificationState));
        logger.debug("iCalendar:\n{}", calendar);
        return message;
    }

    private iCalendar addEvent(iCalendar iCalendar, RenderContext context, iCalendar.Method method)
    {
        ...
        if (target instanceof Target.Room) {
            Target.Room roomTarget = (Target.Room) target;
            meetingName = roomTarget.getName();
            for (Alias alias : roomTarget.getAliases()) {
        ...
        iCalendar.Event event = iCalendar.addEvent(LocalDomain.getLocalDomainName(), id, meetingName);
