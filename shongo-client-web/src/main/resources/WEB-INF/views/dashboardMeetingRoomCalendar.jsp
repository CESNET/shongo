...
<tag:url var="meetingRoomBook" value="<%= ClientWebUrl.WIZARD_MEETING_ROOM_BOOK %>" />
                    ...
                    calendar.fullCalendar('renderEvent',
                            {
                                ...
                                color: '#ff4d4d',
                                id: "stringForDeletion",
                            },
                            true
                    );

                    //look at date and time format
                    var confirmed = confirm("Potvrzeni rezervace v intervalu od " + start.format("dddd, MMMM Do YYYY h:mm:ss") + " do " + end.format("dddd, MMMM Do YYYY h:mm:ss"));
                    if (confirmed == true) {
                        var bookUrl = "${meetingRoomBook}";
                        bookUrl += "?start=";
                        bookUrl += start.toISOString();
                        bookUrl += "&end=";
                        bookUrl += end.toISOString();
                        bookUrl += "&resourceId=";
                        bookUrl += $scope.reservationsFilter.resourceId.id;
                        window.location +=  bookUrl;
                    }
                    calendar.fullCalendar('removeEvents', "stringForDeletion");
