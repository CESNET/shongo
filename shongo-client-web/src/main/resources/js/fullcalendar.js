...
$.extend(DayGrid.prototype, {
	...
	renderSegHtml: function(seg, disableResizing) {
		...
		if (!event.allDay && seg.isStart) {
			timeHtml = '<span class="fc-time">' + htmlEscape(view.getEventTimeText(event, 'LT')) + '</span>';

			timeText = view.getEventTimeText(event);
			fullTimeText = view.getEventTimeText(event, 'LT');
			startTimeText = //view.getEventTimeText(event, 'LT');
			view.getEventTimeText(event.start, null);
		}
...
$.extend(TimeGrid.prototype, {
	...
	renderSegHtml: function(seg, disableResizing) {
		...
		return '<a class="' + classes.join(' ') + '"' +
			...
			'>' +
				'<div class="fc-content">' +
				    // Removed because of diferent formating of dates
					/*(timeText ?
						'<div class="fc-time"' +
...
function generateAgendaAxisFormat(options, langData) {
	return langData.longDateFormat('LT')
		.replace(':mm', '(:mm)')
		TODO.replace(/(\Wmm)$/, '($1)') // like above, but for foreign langs
